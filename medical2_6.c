...
void startfun ()
{
	...
	time_t t = time(NULL);
 	...
 	//Check Last_results.txt file size.
 	
	FILE* fp = fopen("Last_results.txt", "r");
 
	fseek(fp, 0L, SEEK_END); 

	// calculating the size of the file 
	long int resf = ftell(fp); 
	...
	if (flagsum==0)
	{
		...
		if (f == NULL)
		{
			...
			fprintf(f, "Date and time: %02d.%02d.%d %02d:%02d:%02d\n",tm.tm_mday, tm.tm_mon + 1, tm.tm_year + 1900, tm.tm_hour, tm.tm_min, tm.tm_sec);
			fprintf(f, "+--------------------------------------------------------------------------------+\n");
			fprintf(f, "|        |        |        |        |        |        |        |        ||       |\n");
		...
		if (resf < 500000)
		{
			...
			FILE *fp = fopen("Last_results.txt", "a");
			if (fp == NULL)
			{
    			printf("Error opening file.\n");
    			exit(1);
			...
			fprintf(fp, "Date and time: %02d.%02d.%d %02d:%02d:%02d\n",tm.tm_mday, tm.tm_mon + 1, tm.tm_year + 1900, tm.tm_hour, tm.tm_min, tm.tm_sec);
			fprintf(fp, "+--------------------------------------------------------------------------------+\n");
			fprintf(fp, "|        |        |        |        |        |        |        |        ||       |\n");
			fprintf(fp, "|  BASO. | EOSIN. | MYELO. | JUVEN. |  STAB. |  SEG.  | LYMPH. |  MONO. || TOTAL |\n");
			fprintf(fp, "|%08d|%08d|%08d|%08d|%08d|%08d|%08d|%08d||%07d|\n", zk, xk, ck, vk, bk, nk, mk, nextk, sum);
			fprintf(fp, "|--------|--------|--------|--------|--------|--------|--------|--------||       |\n");
			fprintf(fp, "|%07d%%|%07d%%|%07d%%|%07d%%|%07d%%|%07d%%|%07d%%|%07d%%||       |\n", (int)zp, (int)xp, (int)cp, (int)vp, (int)bp, (int)np, (int)mp, (int)nextp);
			fprintf(fp, "|   |C|  |   |V|  |   |B|  |   |N|  |   |M|  |   |<|  |   |>|  |   |?|  ||       |\n");
			fprintf(fp, "+--------------------------------------------------------------------------------+\n");
